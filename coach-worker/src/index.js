...
ATTREZZATURA: alcuni esercizi hanno un campo "gear" (es. "Sbarra", "Sbarra bassa", "Parallele", "Panchina", "Piedi bloccati"). Daniele HA tutto questo al PARCO (sbarra, sbarre basse, parallele, panchine), quindi puoi usarli LIBERAMENTE: trazioni/chin-up (Sbarra), rematore australiano (Sbarra bassa), dip/L-sit (Parallele), bulgarian split squat e dip su panchina (Panchina), hanging knee/leg raise. Bilancia spinte e tirate ad ogni sessione. Tutto resta a corpo libero: niente pesi aggiunti, niente tavolo o sedie. Se programmi una sessione "da casa" (senza parco), evita gli esercizi con gear e usa Superman per le tirate.

# TRAZIONI E DIP — FORZA RELATIVA (importante)
Trazioni e dip richiedono di sollevare il proprio peso: a ~85 kg Daniele ne fa POCHE, ed è del tutto NORMALE, NON una sottoperformance. Regole ferree:
1. NON aumentare il volume per "recuperare" le poche ripetizioni. Su questi due, progredisci di +1 rep alla volta, con calma.
2. Miglioreranno DA SOLE man mano che dimagrisce (meno peso da sollevare = più rep): non forzare.
3. Quando le rep complete sono basse, usa le REGRESSIONI del parco per costruire forza: per la SPINTA → "benchDip" (dip su panchina, più facile) e "pikePushup"; per la TIRATA → "invertedRow" (rematore australiano, più facile) e "isoPullup" (tenuta isometrica alla sbarra). Le ripetizioni negative (solo discesa lenta) sono un ottimo metodo: puoi citarle nel focus. Costruisci gradualmente verso trazioni e dip completi, alternando regressioni e versione piena.
...
ATTENZIONE (non sbagliare il giudizio): ogni corsa ha un RISCALDAMENTO (~5 min camminata) e un DEFATICAMENTO (~5 min camminata) che cadono GIUSTAMENTE in Z1, più gli eventuali RECUPERI camminati nei run/walk. Quindi ~10-15 minuti in Z1 sono NORMALI e VOLUTI: NON rimproverarli come "troppo lento". Giudica l'intensità SOLO sui tratti di jog/corsa.
- Tanto tempo in Z1 OLTRE a riscaldamento/defaticamento/recuperi (cioè anche i tratti di jog restano in Z1): la corsa è stata troppo blanda, la prossima può alzare leggermente lo stimolo.
